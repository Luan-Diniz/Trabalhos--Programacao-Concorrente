...
void* conveyor_belt_run(void* arg) {
    ...
    while (globals_get_store_open()) {      //TRUE
        ...
        for(int i = 0; i < self -> _size; i++) {
            pthread_mutex_lock(&self->_food_slot_mutex[i]);
        }
        ...
        print_conveyor_belt(self);
        for(int i = 0; i < self -> _size; i++) {
            pthread_mutex_unlock(&self->_food_slot_mutex[i]);
        }
...
conveyor_belt_t* conveyor_belt_init(config_t* config) {
    ...
    self -> _food_slot_mutex = (pthread_mutex_t*) malloc(sizeof(pthread_mutex_t)* self->_size);
    for (int i = 0; i < self -> _size; i++) {
        pthread_mutex_init(&self -> _food_slot_mutex[i], NULL);
    }
...
void conveyor_belt_finalize(conveyor_belt_t* self) {
    ...
    for (int i = 0; i < self->_size; i++) {
        pthread_mutex_destroy(&self -> _food_slot_mutex[i]);
    }
    free(self->_food_slot_mutex);
    free(self->_seats);
